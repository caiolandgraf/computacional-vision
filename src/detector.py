...
class GrassDetector:
    ...
    def detect_grass_color_based(self, image: np.ndarray) -> Tuple[np.ndarray, Dict]:
        ...
        """
        is_realtime = self.realtime_params['enabled']

        # Em modo realtime, pula CLAHE (caro: conversão BGR->LAB->BGR)
        if is_realtime:
        ...
        # Em modo realtime, usa ranges fixos (pula calibração adaptativa)
        if is_realtime:
            color_ranges = self.grass_color_ranges
        else:
        ...
        # Combina as máscaras principais
        color_mask = cv2.bitwise_or(green_mask, brown_mask)

        # Máscaras adicionais apenas fora do modo realtime
        if not is_realtime:
        ...
        # Operações morfológicas (simplificadas em realtime)
        kernel_small = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))

        if is_realtime:
            # Versão rápida: apenas uma operação close
        ...
        # Em realtime usa área mínima fixa; fora, ajusta adaptativamente
        base_min_area = self.texture_params['min_area'] // 2
        if is_realtime:
            adjusted_min_area = base_min_area
        elif self.confidence_params['adaptive_min_area']:
        ...
        # Em realtime, estimativa rápida de confiança; fora, cálculo completo
        if is_realtime:
            confidence = min(0.85, coverage_percentage / 50.0)
        else:
        ...
        stats = {
            ...
            'calibration_used': not is_realtime and self.auto_calibration['enabled'],
            'clahe_applied': not is_realtime and initial_coverage < self.auto_calibration['contrast_threshold'],
